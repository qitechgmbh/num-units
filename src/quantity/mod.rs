...
/// - `$dimension`: The complete dimension type (e.g., `SI<0, 0, 0, 0, 0, 1, 0>`)
///
/// # Examples
/// ```rust
...
/// # use num_units::si::SI;
///
/// // Create a dimension for amount of substance using SI
/// quantity!(AmountOfSubstanceDimension, SI<0, 0, 0, 0, 0, 1, 0>);
///
/// // Create a dimension for force (kg⋅m⋅s⁻²) using SI
/// quantity!(ForceDimension, SI<1, 1, -2, 0, 0, 0, 0>);
///
/// // Create a dimensionless quantity using SI
/// quantity!(DimensionlessDimension, SI<0, 0, 0, 0, 0, 0, 0>);
/// ```
#[macro_export]
...
/// - `D`: The dimension type (generated by the `dimensio    #[test]
///
/// Examples
/// ```rust
...
///
/// let length = Quantity::<f64, LengthDimension>::from_raw(5.0);
/// let width = Quantity::<f64, LengthDimension>::from_raw(3.0);
